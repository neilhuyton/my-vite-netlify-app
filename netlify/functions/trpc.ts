...
const ZEPTO_SMTP_USER = process.env.ZEPTO_SMTP_USER || "emailappsmtp.2bcee7c87e4c324";
...
export const appRouter = t.router({
  ...
  verifyEmail: t.procedure
    ...
    .mutation(async ({ input }) => {
      const user = await prisma.user.findFirst({ where: { verificationToken: input.token } });
      if (!user) throw new TRPCError({ code: "BAD_REQUEST", message: "Invalid or expired token" });
